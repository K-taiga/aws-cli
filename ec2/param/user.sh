...
# Variables
# 169.254.169.254はリンクローカルアドレスでAWSのメタデータを取得できるip
...
EC2_NAME=$(aws ec2 describe-instances --region $AWS_REGION --instance-id $INSTANCE_ID \
	# [?フィルタするキー=='値'] ex) 変数あり "Vpcs[?CidrBlock==\`$CIDR_BLOCK`\]",変数なし 'Vpcs[?CidrBlock==`10.0.0./16`]'
    --query 'Reservations[*].Instances[*].Tags[?Key==`Name`].Value' --output text) && echo $EC2_NAME

# TODO: docker,docker-compose,gitを入れるコマンドに変更する
